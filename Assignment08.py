...
# Nikita Daharia,06.08.2020,Modified code to complete assignment 8
# ------------------------------------------------------------------------ #
...
class Product:
    ...
    Nikita Daharia,06.08.2022,Modified code to complete assignment 8
    """

    @staticmethod
    def initialize_data(file_name: lstOfProducts = strFileName) -> lstOfProducts:
        lstOfProductss = [{"Product": "Hair dryer", "Price": "150.50"}, {"Product": "Hair straigtner", "Price": "100.00"}]
...
# Processing  ------------------------------------------------------------- #
class FileProcessor:
    """Processes data to and from a file and a list of products:
    methods:
        save_data_to_file(file_name, list_of_products):
        read_data_from_file(file_name): -> (a list of products)
    changelog: (When,Who,What)
        RRoot,1.1.2030,Created Class
        Nikita Daharia,06.08.2022,Modified code to complete assignment 8
    """
    ...
    def remove_data_from_list(product, list_of_rows):
        ...
        for row in lstOfProducts:
            if (row["Product"] == product):
                lstOfProducts.remove(row)
                print("\nThe",product.lower(),"has been removed from the list!")
        return list_of_rows
...
# The exception handling will handle an error for a non-existing file
try:
    FileProcessor.read_data_from_file( file_name=strFileName, list_of_rows=lstOfProducts)  # read file data
except:
...
while (True):
    ...
    IO.output_current_products_in_list(list_of_rows=lstOfProducts)  # Show current data in the list
    ...
    if choice_str.strip() == '1':  # Add a new Task
        ...
        lstOfProducts = FileProcessor.add_data_to_list(product=product, price=price, list_of_rows=lstOfProducts)
        print("\nThe",product.lower(),"has been added to the list!")
        continue  # to show the menu

    elif choice_str == '2':  # Remove an existing product
        product = IO.input_product_to_remove()
        lstOfProducts = FileProcessor.remove_data_from_list(product=product, list_of_rows=lstOfProducts)
        continue  # to show the menu

    elif choice_str == '3':  # Save Data to a File
        lstOfProducts = FileProcessor.write_data_to_file(file_name=strFileName, list_of_rows=lstOfProducts)
        print("Data Saved!")
        continue  # to show the menu
